...
- JSON Data-Contract "Bible" for Case-Project-UI
  ...
  - Golden Invariants (pin these on every PR)

  - Canonical Types (TypeScript + Zod)
...
1. **Event Categories**: `event.category ∈ {"legal","factual","investigatory"}`.

1. **Auto-Event on Upload**: Ingesting any supported `source.type`must create **exactly one**`event` linked to that source (prototype: no backfills).

1. **Events may exist without a source** (prototype flexibility), but UI **nudges** to link if `category==="legal"`.

1. **Participation Links**: People can be attached to **events** and **sources** as `participants[]`with a`role`.

1. **Cites are read-through**: A CITE always points to a concrete anchor (block/time/page) that is resolvable to a `source`and optionally to an`event`.

1. **Touched-only indexing**: Indexes update **only for edited/created** items (no global rebuilds in prototype).
...
1. Each listed court has ≥1 docket/case number.

1. For district/circuit, `code` is required (1–5 chars matching regex `^``````[A-Za-z0-9-]``````{1,5}$`). For mcoa/msc, `code` is forbidden.

1. `judges[]` is optional and scoped to the specific CaseCourt entry.

1. Max one `isDefault=true` court per case; max one `isDefault=true` case per project.
...
- Ask Codex to:
  ...
  1. Add `tools/index_writer.ts` hooks to event/source write paths.

  1. Add `tools/validate_touched.js` (prototype warnings only) and a `npm run validate:touched` script.
